...
BOOK_DIR_NAME = '{book}'  # one subfolder per bookSlug
...
def chapter_sort_key(ch_str):
    """Convert chapter string to sortable key. Handles 'R', numbers, etc."""
    ch_upper = ch_str.upper()
    if ch_upper.isdigit():
        return (0, int(ch_upper))
    elif ch_upper == 'R':
        return (-1, 0)  # R comes before chapter 1
    else:
        return (1, ch_upper)


def problem_sort_key(prob_str):
    """Convert problem string to sortable key."""
    if prob_str.isdigit():
        return int(prob_str)
    else:
        ...
        return 999999  # fallback for non-numeric
...
def main():
    ...
    for p in SOURCE_DIR.glob('*.har'):
        ...
        print(f"    ✓ Extracted {count} unique problems")
    ...
    print(f"Total books found: {len(by_book)}\n")
